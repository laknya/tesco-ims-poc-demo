...
if [ -z "${REPO}" ]; then
  echo "[FAIL]  Cannot determine repo. Run: export GITHUB_REPOSITORY=owner/repo"
  exit 1
fi

REVIEWER="${GITHUB_ACTOR:-$(gh api user -q .login 2>/dev/null)}"
if [ -z "${REVIEWER}" ]; then
  echo "[FAIL]  Cannot determine GitHub username. Run: export GITHUB_ACTOR=your-username"
  exit 1
fi
...
EOF
  ...
  echo "  [OK]  ${env_name} -- reviewer: ${REVIEWER}, branch: main"
}
...
gh api "/repos/${REPO}/environments" -q '.environments[] | "  [OK]  \(.name)"'
